...
$$
\widehat{y} = \mathbb{1}(\mathbf w^\top \mathbf x + b > 0)
$$
...
$$
\mathbf{w} \leftarrow \mathbf{w} + \eta (y - \widehat{y}) \mathbf{x}
$$
...
$$
\mathbf x \rightarrow \mathbf h^{(1)} \rightarrow \mathbf h^{(2)} \rightarrow \mathbf h^{(3)} \rightarrow \widehat{\mathbf y}
$$
...
Each neuron:
$$
h_j = \sigma(\mathbf{w}_j^\top \mathbf{x} + b_j)
$$
...
![Sigmoid & tanh Activation Functions](/images/sigmoid_tanh_activation_function.png)
...
$$
\text{softmax}(\mathbf z_i) = \frac{e^{z_i}}{\sum_j e^{z_j}}
$$
...
\nabla_{\widehat{\mathbf y}} \mathscr{L} \cdot \text{diag}\left(\sigma'(\mathbf z^{(L + 1)})\right)
$$
